...
// Today we'll be cover if, if/else, if/else if/else statements, ternary expressions and switch statments.

// IF STATEMENTS - Code that runs when a conditional is met.
...
/** TO-DO: Let's expand our previous isItNumber function. Now I want to also return string when we aren't passed a number.
...
// IF/ELSE IF/ELSE = A chain of if/else statemnts to give different results for differnect conditioins.

// if (condition1) {
//     run this code;
// } else if (condition2) {
//     since the first condition was not met, and the second was, I run;
// } else {
 ...
 * I could et my milk from it, but since I don't own either of those: I'll go to the store instead."
 */
...
let hasGoat = false;
// if both are true the first matching condition will run.
...
let action = (doWeHaveMilk) ? "I'm going to make me a bowl of Honey Oh's!" : "Headed out to Randall's. BRB!";
console.log(action);

/** To-DO: Let's look back at the if/else versionof our isItANumber function. Let's refactor our if/else using a ternary expression. */

function isItANumber(val) {
    return (typeof val === "number") ? "That's a number!" : "That's not a number...";
}

// SWITCH STATEMENTS: A statement with multiple cases that will trigger based on the value of a variable.

let color = prompt("What is your favorite color?").toLocaleLowerCase();
...
/** TO-DO: Let's make a switch statement with a prompt for the user to input their favorite sandwich meat. If they like 'olive loaf'
 * we should return an alert to congratulate them on their bold choice, and if they choose 'brisket' we should return
 * a different alert: this one for congratulating them on the 'right' answer. All other responses should trigger an alert stating apathy.
 */

let meatOfChoice = prompt("What's your favorite lunch meat?").toLocaleLowerCase();
